...
const elementCours = document.querySelector('element-cours');
...
function displayCourseData(data) {
    scrollElementCoursTop();

    if (displayMode == 'day') {
        elementCours.renderDay(data, indexJour);

        mettreAJourBoutons();

        handleScrollElementCours();
        elementCours.addEventListener('scroll', handleScrollElementCours);
        ...
        elementCours.renderSemaine(data);

        handleScrollElementCours();
        elementCours.removeEventListener('scroll', handleScrollElementCours);
    }

    modeSwitcher = document.getElementById('mode-switcher');
    modeSwitcher.addEventListener('click', _ => {
        displayMode = displayMode == 'day' ? 'semaine' : 'day';
        displayCourseData(coursData);
    });

    elementCours.classList.remove('blur');
    animation();

    elementCours.focus(); // To scroll with arrows
}

function findNameByUsername(username) {
    const tutor = tutorsData.find(t => t.username === username);
...
function displayArrowElementCours() {
    return elementCours.scrollHeight > elementCours.clientHeight && !elementCours.scrollTop && displayMode == 'day';
}

function handleScrollElementCours() {
    const overflowCours = document.getElementById('overflow-cours');
    if (overflowCours) {
        if (displayArrowElementCours()) {
            overflowCours.setAttribute('visible', 'true');
        } else {
            overflowCours.removeAttribute('visible');
        }
    }
}

function scrollElementCoursBottom() {
    elementCours.scrollTo({
        top: elementCours.scrollHeight,
        behavior: 'smooth'
    });
}

function scrollElementCoursTop() {
    elementCours.scrollTop = 0;
}
    ...
    ws.addEventListener('message', (event) => {
        try {
            ...
            if (message.type === 'courseData') {
                coursData = sortCourseData(message.data.reverse());
                displayCourseData(coursData);
                ...
                currentSemaine = message.data[0];
            } else if (message.type === 'tutorsData') {
                tutorsData = message.data;
            }
        } catch (error) {
...
elementCours.addEventListener('touchstart', (event) => {
    handleStart(event.touches[0].clientX, event.touches[0].clientY);
});

elementCours.addEventListener('mousedown', (event) => {
    isMouseDown = true;
    handleStart(event.clientX, event.clientY);
});

elementCours.addEventListener('touchend', handleEnd);
elementCours.addEventListener('mouseup', handleEnd);
...
logoTram.addEventListener('click', _ => {
    if (canClick) {
        ...
        newImage.src = `images/trams/tram${Math.floor(Math.random() * 10) + 1}.png`;
        // newImage.src = `images/trams/tram10.png`;
    }
});

function jouerSurtom() {
    elementJouerSurtom = document.getElementById('jouer-surtom');
    elementJouerSurtom.classList.remove('confirmation');
    elementJouerSurtom.removeAttribute('onclick');
}
...
setInterval(_ => {
    ...
    min_rotation = 6 * min;
    ms = d.getMilliseconds(); // Get milliseconds
    sec_rotation = 6 * sec + 0.006 * ms; // Angle for seconds with millisecond precision
